    ...
    def __init__(self, inputFile):
        ...
        for line in lines:
            ...
            if not arcsPhase: 
                ...
                s = data[0]
                t = data[1]
                distance = max(0.1,self.haversine(self.nodes[s]["long"],self.nodes[s]["lat"],self.nodes[t]["long"],self.nodes[t]["lat"]))
                #arc metadata
...
class Individual:
    ...
    def insertAdditionalTrack (self, problem, source, target, distance, lanes, maxspeed):
        ...
        self.newTracks[source][target].append({"id": "nt_"+str(self.nextTracksId)   ,"s": source, "t": target, "distance": distance, "lanes": lanes, "maxspeed": maxspeed, "tt": tt })
    ...
    def printDesign (self, problem, outputFilename="output", outputFormat=None):
        ...
        for track in problem.tracks: #regular tracks
            ...
            color = ""
            if self.vehiclesByRoad[track["id"]] <= 7*(track["distance"]/1000.0)*(track["lanes"]+self.tracks[track["id"]]):
                color="green"
            elif self.vehiclesByRoad[track["id"]] <= 11*(track["distance"]/1000.0)*(track["lanes"]+self.tracks[track["id"]]):
                color="yellow"
            elif self.vehiclesByRoad[track["id"]] <= 22*(track["distance"]/1000.0)*(track["lanes"]+self.tracks[track["id"]]):
                color="orange"
            elif self.vehiclesByRoad[track["id"]] <= 28*(track["distance"]/1000.0)*(track["lanes"]+self.tracks[track["id"]]):
                color="purple"
            else:
                color="red"
        ...
        for s in self.newTracks: #non-regular tracks
            for t in self.newTracks[s]:
                for track in self.newTracks[s][t]:
                    edgeLabels[(s, t)] = "New: ("+str(track["distance"])+"m, "+str(track["maxspeed"])+"km/h) x "+str(track["lanes"])+"."
        
        
        #nx.draw_networkx(graph, pos=nodePos,  arrows=True, arrowstyle='-|>', with_labels=False, node_size=0, edge_color=edgeColors)
        nx.draw(graph, pos=nodePos,  arrows=True, arrowstyle='-|>', with_labels=False, node_size=0, edge_color=edgeColors)
    ...
    def fitness(self, problem, individual):
                ...
                for target in individual.newTracks[source]:
                    for i in range(len(individual.newTracks[source][target])):
                        ...
                        if "maxspeed" in individual.newTracks[source][target][i]:
                            speed = track["maxspeed"]
                        individual.newTracks[source][target][i]["tt"] =  (individual.newTracks[source][target][i]["distance"]/1000.0 * (1/speed)) / nminTT
            ...
            if is_t_source_adjList == True:                 
                for tl in problem.adjLst[t]:
                    for track2 in problem.adjLst[t][tl]:
                        ...
                        if "maxspeed" in track2:
                            speed = track2["maxspeed"]
                        pMatrix[rid][rid2] += (1.0-pMatrix[rid][rid])*(((track2["lanes"]+individual.tracks[track2["id"]]) * (1/speed))/ (lanes) )
        ...
        for i in range(len(problem.tracks)): #number of vehicles for regular tracks
            ...
            if rid not in individual.vehiclesByRoad:
                individual.vehiclesByRoad[rid] = (self.vehiclesByRoad*(track["distance"]/1000.0)*(track["lanes"]+individual.tracks[rid]))*pMatrix[rid][rid]
                ...
                for tl in problem.adjLst[t]:
                    for track2 in problem.adjLst[t][tl]:
                        ...
                        if rid2 not in individual.vehiclesByRoad:
                            individual.vehiclesByRoad[rid2] = (self.vehiclesByRoad*(track2["distance"]/1000.0)*(track2["lanes"]+individual.tracks[rid2]))*pMatrix[rid2][rid2]
                        individual.vehiclesByRoad[rid2] += pMatrix[rid][rid2]*(self.vehiclesByRoad*(track["distance"]/1000.0)*(track["lanes"]+individual.tracks[rid]))
            if is_t_source_adjIndividual == True:
                for tl in individual.newTracks[t]:
                    for track2 in individual.newTracks[t][tl]:
                        rid2 = track2["id"]
                        if rid2 not in individual.vehiclesByRoad:
                            individual.vehiclesByRoad[rid2] = (self.vehiclesByRoad*(track2["distance"]/1000.0)*(track2["lanes"]))*pMatrix[rid2][rid2]
                        individual.vehiclesByRoad[rid2] += pMatrix[rid][rid2]*(self.vehiclesByRoad*(track["distance"]/1000.0)*(track["lanes"]+individual.tracks[rid]))
        ...
        for s in individual.newTracks:#number of vehicles for non-regular tracks                
            for t in individual.newTracks[s]: 
                for track in individual.newTracks[s][t]: 
                    ...
                    if rid not in individual.vehiclesByRoad:
                        individual.vehiclesByRoad[rid] = (self.vehiclesByRoad*(track["distance"]/1000.0)*(track["lanes"]))*pMatrix[rid][rid]
                    ...
                    if is_t_source_adjList == True:
                        for tl in problem.adjLst[t]:
                            for track2 in problem.adjLst[t][tl]:
                                ...
                                if rid2 not in individual.vehiclesByRoad:
                                    individual.vehiclesByRoad[rid2] = (self.vehiclesByRoad*(track2["distance"]/1000.0)*(track2["lanes"]+individual.tracks[rid2]))*pMatrix[rid2][rid2]
                                individual.vehiclesByRoad[rid2] += pMatrix[rid][rid2]*(self.vehiclesByRoad*(track["distance"]/1000.0)*(track["lanes"]))
                    if is_t_source_adjIndividual == True:
                        for tl in individual.newTracks[t]:
                            for track2 in individual.newTracks[t][tl]:
                                rid2 = track2["id"]
        ...
        for track in problem.tracks: #regular tracks
            rid = track["id"]
            dens = Utils.IndetZero(individual.vehiclesByRoad[rid] , ((track["distance"]/1000.0)*(track["lanes"]+individual.tracks[rid])) )
            densSum += dens
            densCount += 1
        for s in individual.newTracks:
            for t in individual.newTracks[s]: 
                for track in individual.newTracks[s][t]: 
                    rid = track["id"]
                    dens = Utils.IndetZero(individual.vehiclesByRoad[rid] , ((track["distance"]/1000.0)*(track["lanes"])) ) 
                    densSum += dens
                    densCount += 1
